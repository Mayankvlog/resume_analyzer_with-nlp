...
connection = pymysql.connect(host='localhost', user='root', password='Mayank@#03')
...
def run():
    ...
    if choice == 'Normal User':
            ...
            if resume_data:
                ...
                for i in resume_data['skills']:
                    ...
                    if i.lower() in ds_keyword:
                        ...
                        st.markdown(
                            '''<h4 style='text-align: left; color: #1ed760;'>Smart-resume-analyzer-using_AI</h4>''',
                            unsafe_allow_html=True)
        ...
        if st.button('Login'):
            if ad_user == 'Mayank_singh' and ad_password == 'mksing2023':
                st.success("Welcome Achouak")
                # Display Data
                ...
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                 ...
                                                 'Recommended Course'])
                st.dataframe(df)
                st.markdown(get_table_download_link(df, 'Users_Data.csv', 'Download Report'), unsafe_allow_html=True)
                ## Admin Side Data
